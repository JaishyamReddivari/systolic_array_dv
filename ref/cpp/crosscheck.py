#!/usr/bin/env python3
"""Three-way INT8 matmul cross-check: C++ ref vs NumPy golden vs cycle-accurate sim.

Feeds identical random INT8 matrix pairs to all three and confirms they agree.
Three independent implementations guard against a single golden being silently wrong.
Build the C++ binary first, then run from the repo root:
...
import subprocess
import numpy as np
# sys.path insert makes the python/ models importable regardless of CWD.
HERE = os.path.dirname(os.path.abspath(__file__))
REF_DIR = os.path.dirname(HERE)
sys.path.insert(0, os.path.join(REF_DIR, "python"))
from golden import matmul_int8                 # noqa: E402  (import intentionally after path setup)
